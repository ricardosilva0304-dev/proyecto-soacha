...
const nav = [
    ...
    {
        path: '/historial', label: 'Historial', sub: 'Ventas realizadas',
        icon: <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
    },
    {
        path: '/reportes', label: 'Reportes', sub: 'Métricas y exportar',
...
const navSecondary = [
    {
        path: '/configuracion', label: 'Configuración', sub: 'Ajustes del sistema',
...
export default function Sidebar({ isOpen, onClose }) {
    ...
    const NavItem = ({ path, label, sub, icon }) => {
        const active = location.pathname === path
        return (
            <Link to={path} className={`nav-item ${active ? 'active' : ''}`} onClick={handleNav}>
        ...
        <>
            ...
            <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
                ...
                <div style={{ padding: '20px 16px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                            <div style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, background: 'linear-gradient(135deg,#c8f560,#a8d940)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(200,245,96,0.35)' }}>
                        ...
                        <button onClick={onClose} className="sidebar-close-btn" style={{ display: 'none', width: 32, height: 32, borderRadius: 9, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--ink-10)', flexShrink: 0 }}>
                ...
                <nav style={{ flex: 1, padding: '14px 12px', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
                    ...
                    {navSecondary.map(item => <NavItem key={item.path} {...item} />)}
                </nav>
                ...
                <div style={{ padding: '12px', borderTop: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                    <div style={{ borderRadius: 13, padding: '11px 12px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, background: 'linear-gradient(135deg,#c8f560,#a8d940)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#080c0a', fontSize: 13, fontWeight: 900 }}>
                            {(usuario?.nombre?.[0] || usuario?.email?.[0] || 'A').toUpperCase()}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ fontSize: 13, fontWeight: 700, color: '#fff', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', letterSpacing: '-0.01em' }}>{usuario?.nombre || usuario?.email || 'Administrador'}</p>
                            <p style={{ fontSize: 10.5, color: 'var(--ink-20)', marginTop: 1 }}>Admin · Activo</p>
                        </div>
                        <button onClick={handleLogout} title="Cerrar sesión" style={{ width: 28, height: 28, borderRadius: 8, border: 'none', background: 'rgba(255,107,107,0.12)', color: '#ff9b9b', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: 'all 0.15s' }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 4px 2px' }}>
                        ...
                        <span style={{ fontSize: 10, color: 'var(--ink-30)', letterSpacing: '0.02em' }}>Soacha · UNIMINUTO 2025</span>
                    </div>
                </div>
            </aside>

            <style>{`@media(max-width:1023px){.sidebar-close-btn{display:flex!important;}}`}</style>
        </>
    )
}
