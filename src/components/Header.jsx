...
const routes = {
    ...
    '/historial':      { title: 'Historial',         sub: 'Ventas realizadas' },
    '/reportes':       { title: 'Reportes',          sub: 'Métricas y exportar' },
    ...
    return (
        <header className="app-header">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: 1 }}>
                <button onClick={onMenuClick} id="menu-btn" style={{ display: 'none', width: 38, height: 38, borderRadius: 10, border: '1.5px solid rgba(8,12,10,0.1)', background: 'var(--surface)', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--ink-50)', flexShrink: 0, transition: 'all 0.15s' }}>
                    <svg width="17" height="17" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
                </button>
                <div style={{ minWidth: 0 }}>
                    <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(16px,3vw,19px)', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.03em', lineHeight: 1.15, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{info.title}</h1>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                {!narrow && (
                    <div style={{ padding: '5px 12px', borderRadius: 10, background: 'var(--surface)', border: '1px solid rgba(8,12,10,0.08)', textAlign: 'right', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: narrow ? '8px' : '6px 12px', borderRadius: 10, background: 'rgba(200,245,96,0.12)', border: '1px solid rgba(200,245,96,0.25)' }}>
                    <span className="pulse-dot" />
                    {!narrow && <span style={{ fontSize: 12, fontWeight: 700, color: '#4a7020', letterSpacing: '-0.01em' }}>En línea</span>}
                </div>
                <button onClick={handleLogout} title="Cerrar sesión" style={{ width: 36, height: 36, borderRadius: 10, border: '1.5px solid rgba(255,107,107,0.2)', background: 'rgba(255,107,107,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#c53030', flexShrink: 0, transition: 'all 0.15s' }}
